...
customFit <- XIFF::buildMachineLearning(
  ...
  .extraClass = "CustomModel",
  .otherParams = list(thresholds = c(-Inf, -3, -2, -1, 0, 1, 2, 3, Inf))
)

# Having that function allows to run getDataForModel and validateModel
...
validationData <- getDataForModel(validationSet, customFit)
...
# mlGetTableData mlGetTpmData mlGenerateExpressionPlot
