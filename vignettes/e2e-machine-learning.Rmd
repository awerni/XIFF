...
```{r}
testData <- getDataForModel(assignment = testSet, features = defaultFit)
annoFocus <- CLIFF::getCellLineAnno("human") %>% dplyr::filter(celllinename %in% unlist(testSet))


predictionSummary <- getPredictionSummary(
  items = testData$celllinename,
  preds = predict(defaultFit, newdata = testData),
  refs = testData$class,
...
```{r}
makeMlModelPlots(defaultFit, testSet, annoFocus)
```

### Glmnet

```{r}
makeMlModelPlots(glmnetFit, testSet, annoFocus)
```
...
makeMlModelPlots(svmFit, testSet, annoFocus)
```
...
if(XIFF::packageInstalled("rpart")) {
  ...
  makeMlModelPlots(rpartFit, testSet, annoFocus)
}
...
makeMlModelPlots(grepFit, testSet, annoFocus)
